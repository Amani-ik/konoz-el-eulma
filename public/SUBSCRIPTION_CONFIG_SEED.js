/**
 * Seed document for Firestore: config/subscription
 * Create this document in Firebase Console or via Admin SDK.
 */
export const SUBSCRIPTION_CONFIG_SEED = {
  payment: {
    ccp: "37233694525777",
    baridimob: "S483823002W00000",
    holder: "كنوز العلمة",
  },
  subscriptionDays: 30,
  plans: [
    {
      id: "single",
      name: "حي واحد",
      subtitle: "Single Niche",
      price: 3500,
      maxDistricts: 1,
      active: true,
      order: 1,
      features: [
        "وصول كامل إلى حي واحد من اختيارك",
        "خريطة تفاعلية لجميع الموردين",
        "دفعة واحدة — اشتراك شهري",
      ],
    },
    {
      id: "dual",
      name: "حيّان",
      subtitle: "Dual Niche",
      price: 5000,
      maxDistricts: 2,
      active: true,
      order: 2,
      features: [
        "وصول إلى حيّين من اختيارك",
        "خريطة تفاعلية لجميع الموردين",
        "دفعة واحدة — اشتراك شهري",
      ],
    },
    {
      id: "all",
      name: "جميع الأحياء",
      subtitle: "All Districts",
      price: 7500,
      maxDistricts: 7,
      mostPopular: true,
      active: true,
      order: 3,
      features: [
        "وصول شامل لجميع الأسواق السبعة",
        "جميع المميزات Premium",
        "دفعة واحدة — اشتراك شهري",
      ],
    },
  ],
  districts: [
    {
      id: "Kankari",
      name: "سوق الجملة للخردوات ومواد البناء",
      emoji: "🔧",
      active: true,
    },
    {
      id: "kitchen",
      name: "سوق الجملة للأدوات والأواني المنزلية",
      emoji: "🍶",
      active: true,
    },
    {
      id: "sports",
      name: "سوق الجملة لمستلزمات الرياضة",
      emoji: "⚽",
      active: true,
    },
    {
      id: "Decoration",
      name: "سوق الجملة لمستلزمات الديكور والزينة",
      emoji: "🎍",
      active: true,
    },
    {
      id: "electronics",
      name: "سوق الجملة للأجهزة الإلكترونية",
      emoji: "💻",
      active: true,
    },
    {
      id: "Electricity",
      name: "سوق الجملة للمستلزمات الكهربائية",
      emoji: "⚡",
      active: true,
    },
    { id: "Toys", name: "سوق الجملة للألعاب", emoji: "🎠", active: true },
  ],
};

/**
 * User payment subcollection path: users/{uid}/payments/{paymentId}
 *
 * Pending payment document (created on receipt submit):
 * {
 *   receiptUrl: string,
 *   planId: string,
 *   planName: string,
 *   amount: number,
 *   districts: array,
 *   status: "pending",
 *   subscriptionType: "monthly_one_time",
 *   submittedAt: Timestamp
 * }
 *
 * Approved payment document (updated by admin):
 * {
 *   status: "approved",
 *   approvedAt: Timestamp,
 *   ...
 * }
 */
